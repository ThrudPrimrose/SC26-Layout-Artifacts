...
def measure_bandwidth(threads):
    ...
    cmd = f"g++ -O3 -march=native -mtune=native fno-vect-cost-model -DNOHUGEPAGE-fopenmp -o {bbin} {src}"
...
def compile_kernels(force=False):
    if Path(BINARY_KERN).exists() and not force:
        ...
        return True
    cmd = f"g++ -O3 -march=native -mtune=native fno-vect-cost-model -DNOHUGEPAGE-fopenmp -o {BINARY_KERN} transpose_cpu.cpp"
...
def compile_hptt(force=False):
    ...
    cmd = (f"g++ -O3 -march=native -mtune=native fno-vect-cost-model -DNOHUGEPAGE-fopenmp {hptt_flags} "
