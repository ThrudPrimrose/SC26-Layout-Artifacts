...
template<int TX, int TY, int SB>
__global__ void tr_smem_blk_swiz(const float* __restrict__ in, float* __restrict__ out, int N) {
    const int BW = blockDim.x * TX, BH = blockDim.y * TY;
    extern __shared__ float sm[];
    int c0 = blockIdx.x * BW, r0 = blockIdx.y * BH;
    int tid = threadIdx.y * blockDim.x + threadIdx.x, tot = blockDim.x * blockDim.y;
...
void d_smem_blk_swiz(int tx,int ty,int sb,dim3 g,dim3 b,size_t sm,const float*in,float*out,int N){
    if     (sb== 8){D3_BODY(tr_smem_blk_swiz, 8)}
    else if(sb==16){D3_BODY(tr_smem_blk_swiz,16)}
    else if(sb==32){D3_BODY(tr_smem_blk_swiz,32)}
    else if(sb==64){D3_BODY(tr_smem_blk_swiz,64)}
    else if(sb==128){D3_BODY(tr_smem_blk_swiz,128)}
    else{fprintf(stderr,"Bad SB=%d\n",sb);exit(1);}
}
...
int main(int argc, char** argv) {
    if (argc < 8) {
        ...
        fprintf(stderr,"  variant: 0=naive 1=blocked 2=smem 3=smem_blk 4=smem_pad 5=smem_swiz 6=smem_blk_swiz 7=smem_pad_blk\n");
        return 1;
    }
    ...
    float *dI, *dO;
    CHECK(cudaMalloc(&dI, bytes)); CHECK(cudaMalloc(&dO, bytes));
    ...
    if (is_blocked) {
        ...
        CHECK(cudaMemcpy(dI, hb, bytes, cudaMemcpyHostToDevice));
    } else {
        CHECK(cudaMemcpy(dI, h, bytes, cudaMemcpyHostToDevice));
    }
    ...
    auto launch = [&](const float* in, float* out) {
        switch(VAR) {
            ...
            case 6: d_smem_blk_swiz(TX,TY,SB,  grid,block,sm_np,in,out,N); break;
    ...
    for (int i = 0; i <= REPS; i++) CHECK(cudaEventCreate(&ev[i]));
    for (int i = 0; i < REPS; i++) {
        CHECK(cudaEventRecord(ev[i]));
        launch(dI, dO);
    }
    CHECK(cudaEventRecord(ev[REPS]));
    CHECK(cudaEventSynchronize(ev[REPS]));

    float* ims = (float*)malloc(REPS * sizeof(float));
    float tot = 0;
    for (int i = 0; i < REPS; i++) {
        CHECK(cudaEventElapsedTime(&ims[i], ev[i], ev[i+1]));
        tot += ims[i];
    }

    CHECK(cudaMemcpy(h, dO, bytes, cudaMemcpyDeviceToHost));
    double cksum = 0;
    ...
    for (int i = 0; i <= REPS; i++) CHECK(cudaEventDestroy(ev[i]));
    free(ev); free(ims); free(h); if(hb) free(hb);
    CHECK(cudaFree(dI)); CHECK(cudaFree(dO));
    return 0;
}
