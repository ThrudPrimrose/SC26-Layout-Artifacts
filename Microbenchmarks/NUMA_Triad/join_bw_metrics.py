...
def join_with_cost(rt_best, cost_df, hw_type):
    ...
    for _, r in rt_best[rt_best["hw_type"] == hw_type].iterrows():
        ...
        if crows.empty:
            mu, delta, sigma = np.nan, np.nan, np.nan
            loop_order = "?"
        else:
            c = crows.iloc[0]
            mu = c["mu"]
            delta = c["delta"]
            sigma = c["sigma"] if "sigma" in c.index else mu * delta
...
def print_table(joined, platform_label):
    ...
    for hw in ["cpu", "gpu"]:
        ...
        # CPU ranks by σ (total weighted traffic, lower = better)
        # GPU ranks by μ (block count, lower = better)
        if hw == "cpu":
            metric_col = "sigma"
            metric_sym = "σ"
        else:
        ...
        if hw == "cpu":
            print(f"  {'#bw':>3} {'#'+metric_sym:>4} {'Variant':<8} {'Distribution':<14} {'Schedule':<18} "
        ...
        for _, r in sub.iterrows():
            ...
            if hw == "cpu":
                sig_s = f"{r['sigma']:.1f}" if np.isfinite(r['sigma']) else "---"
                print(f"  {r['bw_rank']:>3} {r['metric_rank']:>4}{match} {r['variant']:<8} {r['dist']:<14} {r['schedule']:<18} "
                      f"{r['loop_order']:<14} {bw_s:>10} {std_s:>6} "
                      f"{mu_s:>8} {delta_s:>10} {sig_s:>12}")
            else:
