...
static void c_aos(C2 *__restrict__ in, C2 *__restrict__ out, int64_t n) {
    #pragma omp parallel for schedule(static)
...
template<int VL>
static void c_aosoa(C2V<VL> *__restrict__ in, C2V<VL> *__restrict__ out, int64_t n) {
    int64_t nblks = n / VL;
    #pragma omp parallel for schedule(static)
    for (int64_t b = 0; b < nblks; b++)
        #pragma omp simd
...
int main() {
    ...
    printf("conj (inplace): N=%lldM  block=%ld  runs=%d  omp_threads=%d  dtype=double\n",
