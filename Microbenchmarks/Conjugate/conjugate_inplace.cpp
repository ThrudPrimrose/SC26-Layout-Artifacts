...
constexpr int64_t N    = 1 << 27;   // 32M elements
...
static void c_aos(C2* __restrict__ in, C2* __restrict__ out, int n) {
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < n; i++) in[i].im = -in[i].im;
}

static void c_soa(double* __restrict__ ri, double* __restrict__ ii,
                  double* __restrict__ ro, double* __restrict__ io, int n) {
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < n; i++) { ii[i] = -ii[i]; }
}

template<int VL>
static void c_aosoa(C2V<VL>* __restrict__ in, C2V<VL>* __restrict__ out, int n) {
    int nblks = n / VL;
    #pragma omp parallel for schedule(static)
