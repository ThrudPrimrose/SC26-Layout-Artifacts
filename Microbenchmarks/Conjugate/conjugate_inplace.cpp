...
/* ═══ Cache flush: write a buffer larger than LLC to evict all lines ═══ */
constexpr int64_t FLUSH_N = 1 << 26;  // 64M doubles = 512 MB > any LLC
...
template<int VL> static void c_aosoa(C2V<VL> *__restrict__ in, int64_t n) {
    int64_t nblks = n / VL;
    #pragma omp parallel for schedule(static)
    for (int64_t b = 0; b < nblks; b++)
        #pragma omp simd
        for (int l = 0; l < VL; l++) in[b].im[l] = -in[b].im[l];
}

/* BW = 2N×8 */
static FILE *csv;
static double bw(double ms) { return N * sizeof(double) / (ms * 1e6); }
...
int main() {
    ...
    printf("conj (inplace): N=%lldM  runs=%d  threads=%d\n",
