                        ...
                        double* hC, double ref_cs, double ref_cs_rm)
{
    ...
    for (int w = 0; w < NWARMUP; w++) {
        CUDA_CHECK(hipMemset(dC, 0, total * sizeof(double)));
        cfg.launch(dA, dB, dC);
    }
    CUDA_CHECK(hipDeviceSynchronize());

    /* correctness */
    CUDA_CHECK(hipMemcpy(hC, dC, total * sizeof(double), hipMemcpyDeviceToHost));
    double cs = 0.0;
    ...
    for (int r = 0; r < NREP; r++) {
        ...
        CUDA_CHECK(hipEventElapsedTime(&times[r], t0, t1));
...
int main(int argc, char** argv)
{
    ...
    CUDA_CHECK(hipMalloc(&dB_cm, bytes));
    CUDA_CHECK(hipMalloc(&dB_rm, bytes));
    ...
    CUDA_CHECK(hipFree(dB_cm));
    CUDA_CHECK(hipFree(dB_rm));
