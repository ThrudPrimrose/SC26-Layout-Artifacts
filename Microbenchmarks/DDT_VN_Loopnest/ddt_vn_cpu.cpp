...
struct SoA_D {
  int N, Nc, Nv, nlev;
  size_t se;
  double *vt, *vn_ie, *z_kin_hor_e, *ddqz_z_full_e, *coeff_gradekin, *c_lin_e,
      *f_e, *z_ekinh, *z_w_con_c_full, *zeta;
...
struct AoS_D {
  ...
  double *aos_e3d, *vn_ie, *z_kin_hor_e, *f_e, *aos_e2d;
  int *aos_conn;
  double *aos_cell, *zeta, *out;
  void build(const Stg &s, bool nf_) {
    N = s.N;
    Nc = s.Nc;
    Nv = s.Nv;
...
struct Grp_D {
  int N, Nc, Nv, nlev;
  size_t se, szA, szD;
  double *grpA, *grpD, *z_kin_hor_e, *ddqz_z_full_e, *f_e, *coeff_gradekin,
      *c_lin_e, *zeta;
  int *cell_idx, *vert_idx;
  double *out;
  void build(const Stg &s, bool nf_) {
...
int main(int argc, char *argv[]) {
  ...
  for (int nlev : nlevs) {
    ...
    for (int di = 0; di < 4; di++) {
      ...
      gen_conn(cl, N, Nc, dist, rng);
      gen_conn(vl, N, Nv, dist, rng);
      Stg stg;
  ...
  printf(
      "  %zu nlevs × 4 dists × 8 layouts × 2 collapse × %d reps = %zu rows\n",
      nlevs.size(), NRUNS, nlevs.size() * 4 * 8 * 2 * NRUNS);
  return 0;
}