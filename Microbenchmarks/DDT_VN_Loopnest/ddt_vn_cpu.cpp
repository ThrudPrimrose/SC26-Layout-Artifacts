...
template <typename T> static T *na(size_t n) {
  void *p = mmap(0, n * sizeof(T), PROT_READ | PROT_WRITE,
...
struct Grp_D {
  int N, Nc, Nv, nlev;
  size_t se, szA, szD;
  double *__restrict__ grpA, *__restrict__ grpD,
                                 *__restrict__ z_kin_hor_e,
                                     *__restrict__ ddqz_z_full_e,
                                         *__restrict__ f_e,
                                             *__restrict__ coeff_gradekin,
                                                 *__restrict__ c_lin_e,
  ...
  void build(const Stg &s, bool nf_) {
    ...
    double *tcg = new double[N * 2];
    pack_pairs(tcg, s.cg, N);
    coeff_gradekin = nr1(tcg, N * 2);
    delete[] tcg;
    double *tcl = new double[N * 2];
    pack_pairs(tcl, s.cl, N);
    c_lin_e = nr1(tcl, N * 2);
    delete[] tcl;
    int *tci = new int[N * 2];
    pack_pairs_i(tci, s.ci, N);
    cell_idx = nri(tci, (size_t)N * 2);
    delete[] tci;
    int *tvi = new int[N * 2];
    pack_pairs_i(tvi, s.vi, N);
    vert_idx = nri(tvi, (size_t)N * 2);
    delete[] tvi;
    out = na<double>(se);
    _Pragma("omp parallel for schedule(static)") for (size_t i = 0; i < se; i++)
        out[i] = 0;
...
#define PG(d)                                                                  \
  ...
  const double *__restrict__ coeff_gradekin = (d).coeff_gradekin,              \
                             *__restrict__ c_lin_e = (d).c_lin_e;              \
