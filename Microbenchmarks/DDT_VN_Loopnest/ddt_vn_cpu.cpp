...
int main(int argc,char*argv[]){
    ...
    for(int nlev:nlevs){size_t se=(size_t)N*nlev;
        for(int di=0;di<4;di++){CellDist dist=(CellDist)di;printf("=== nlev=%d dist=%s ===\n",nlev,dist_names[di]);
            int*cl=new int[N*2],*vl=new int[N*2];gen_conn(cl,N,Nc,dist,rng);gen_conn(vl,N,Nv,dist,rng);
    ...
    printf("  %zu nlevs × 4 dists × 8 layouts × 2 collapse × %d reps = %zu rows\n",
           nlevs.size(),NRUNS,nlevs.size()*4*8*2*NRUNS);return 0;}