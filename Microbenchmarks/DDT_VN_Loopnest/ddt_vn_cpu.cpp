 ...
 *   grpA[NA=2, N, nlevp1]  = {vt, vn_ie}
 *   grpD[ND=2, N_c, nlev]  = {z_ekinh, z_w_con_c_full}
 *   Standalone: z_kin_hor_e, ddqz_z_full_e, f_e, zeta, out
 *   Packed 2D (coef[NC=4]={cg0,cg1,cl0,cl1}, conn[NCONN=4]={ci0,ci1,vi0,vi1}):
 *     coef[4*N], conn[4*N] (int)
 *
 * Full AoS:
 ...
 *
 * Compile: g++ -O3 -fopenmp -march=native -std=c++17 bench_ddt_vn_cpu_sweep.cpp
 * -o bench
 */
...
/* ─── NUMA ────────────────────────────────────────────────────── */
template <typename T> static T *na(size_t n) {
  if (n == 0)
    n = 1; /* mmap rejects length 0 */
  size_t bytes = n * sizeof(T);
  void *p = mmap(0, bytes, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS,
                 -1, 0);
  if (p == MAP_FAILED) {
    perror("mmap");
...
template <typename T> static void nf_(T *p, size_t n) {
  if (n == 0)
    n = 1;
  munmap(p, n * sizeof(T));
}
...
static double *nr2(const double *s, int N, int K) {
  ...
  _Pragma("omp parallel for schedule(static)") for (int jk = 0; jk < K; jk++)
...
template <typename T> static T *nri(const T *s, size_t n) {
  T *d = na<T>(n);
  _Pragma("omp parallel for schedule(static)") for (size_t i = 0; i < n; i++)
      d[i] = s[i];
  return d;
...
static double *nrA(const double *s, int N, int np) {
  ...
  double *d = na<double>(n);
  _Pragma("omp parallel for schedule(static)") for (int jk = 0; jk < np; jk++) {
...
static double *nrD(const double *s, int Nc, int nl) {
  ...
  double *d = na<double>(n);
  _Pragma("omp parallel for schedule(static)") for (int jk = 0; jk < nl; jk++) {
...
static double *nr2nf(const double *s, int N, int K) {
  size_t n = (size_t)N * K;
  double *d = na<double>(n);
  _Pragma("omp parallel for schedule(static)") for (int je = 0; je < N; je++)
...
static double *nrDnf(const double *s, int Nc, int nl) {
  ...
  double *d = na<double>(n);
  _Pragma("omp parallel for schedule(static)") for (int c = 0; c < Nc; c++) {
...
static double *nraAnf(const double *s, int N, int np) {
  ...
  size_t n = (size_t)t * NA * VW * np;
  double *d = na<double>(n);
  _Pragma("omp parallel for schedule(static)") for (int ti = 0; ti < t; ti++) {
...
static inline int IX(int je, int n, int N) {
  return je + n * N;
} /* stride-N pairs (SoA) */
...
/* Grouped je-first */
static inline int IA(int f, int je, int jk, int N) {
  return f + NA * (je + N * jk);
}
static inline int ID(int f, int c, int jk, int Nc) {
  return f + ND * (c + Nc * jk);
}
/* Full AoS je-first */
static inline int IE3(int f, int je, int jk, int N) {
  return f + NE3 * (je + N * jk);
}
...
static inline int IAnf(int f, int je, int jk, int K) {
  return f + NA * (jk + K * je);
}
static inline int IDnf(int f, int c, int jk, int K) {
  return f + ND * (jk + K * c);
}
static inline int IE3nf(int f, int je, int jk, int K) {
  return f + NE3 * (jk + K * je);
}
...
static inline size_t szAao(int N, int K) {
  return (size_t)((N + VW - 1) / VW) * NA * VW * K;
}
static inline size_t szDao(int Nc, int K) {
  return (size_t)((Nc + VW - 1) / VW) * ND * VW * K;
}
...
/* ── Grouped je-first (coef[NC=4], conn[NCONN=4]) ── */
#define GRP_BODY()                                                             \
  {                                                                            \
...
#define GRP_NF_BODY()                                                          \
  {                                                                            \
    ...
    double vt_e = grpA[IAnf(A_VT, je, jk, np)],                                \
           vk = grpA[IAnf(A_VN_IE, je, jk, np)],                               \
           vk1 = grpA[IAnf(A_VN_IE, je, jk + 1, np)];                          \
    double ekin_e = z_kin_hor_e[I2nf(je, jk, nlev)],                           \
           ddqz_e = ddqz_z_full_e[I2nf(je, jk, nlev)], fe_e = f_e[je];         \
    double eh0 = grpD[IDnf(D_EKINH, ci0, jk, nlev)],                           \
           eh1 = grpD[IDnf(D_EKINH, ci1, jk, nlev)];                           \
    double w0 = grpD[IDnf(D_WCON, ci0, jk, nlev)],                             \
           w1 = grpD[IDnf(D_WCON, ci1, jk, nlev)];                             \
    double zeta0 = zeta[I2nf(vi0, jk, nlev)],                                  \
           zeta1 = zeta[I2nf(vi1, jk, nlev)];                                  \
    STENCIL_EXPR(I2nf(je, jk, nlev))                                           \
  }

/* ── AoSoA-16 je-first (coef[NC=4], conn[NCONN=4]) ── */
#define AOSOA_BODY()                                                           \
  {                                                                            \
    int ci0 = conn[ICN(CN_CI0, je)], ci1 = conn[ICN(CN_CI1, je)];              \
    int vi0 = conn[ICN(CN_VI0, je)], vi1 = conn[ICN(CN_VI1, je)];              \
    ...
    double vt_e = grpA[IAao(A_VT, je, jk, N)],                                 \
           vk = grpA[IAao(A_VN_IE, je, jk, N)],                                \
           vk1 = grpA[IAao(A_VN_IE, je, jk + 1, N)];                           \
    double ekin_e = z_kin_hor_e[I2(je, jk, N)],                                \
           ddqz_e = ddqz_z_full_e[I2(je, jk, N)], fe_e = f_e[je];              \
    double eh0 = grpD[IDao(D_EKINH, ci0, jk, N_c)],                            \
           eh1 = grpD[IDao(D_EKINH, ci1, jk, N_c)];                            \
    double w0 = grpD[IDao(D_WCON, ci0, jk, N_c)],                              \
           w1 = grpD[IDao(D_WCON, ci1, jk, N_c)];                              \
    double zeta0 = zeta[I2(vi0, jk, N_v)], zeta1 = zeta[I2(vi1, jk, N_v)];     \
    STENCIL_EXPR(I2(je, jk, N))                                                \
  }
...
#define AOSOA_NF_BODY()                                                        \
  {                                                                            \
    ...
    double vt_e = grpA[IAaonf(A_VT, je, jk, np)],                              \
           vk = grpA[IAaonf(A_VN_IE, je, jk, np)],                             \
           vk1 = grpA[IAaonf(A_VN_IE, je, jk + 1, np)];                        \
    double ekin_e = z_kin_hor_e[I2nf(je, jk, nlev)],                           \
           ddqz_e = ddqz_z_full_e[I2nf(je, jk, nlev)], fe_e = f_e[je];         \
    double eh0 = grpD[IDaonf(D_EKINH, ci0, jk, nlev)],                         \
           eh1 = grpD[IDaonf(D_EKINH, ci1, jk, nlev)];                         \
    double w0 = grpD[IDaonf(D_WCON, ci0, jk, nlev)],                           \
           w1 = grpD[IDaonf(D_WCON, ci1, jk, nlev)];                           \
    double zeta0 = zeta[I2nf(vi0, jk, nlev)],                                  \
           zeta1 = zeta[I2nf(vi1, jk, nlev)];                                  \
    STENCIL_EXPR(I2nf(je, jk, nlev))                                           \
  }
...
static void flush() {
  double *A = fb0, *B = fb1;
  for (int s = 0; s < FS; s++) {
    _Pragma("omp parallel for schedule(static)") for (int i = 1; i < FN - 1;
                                                      i++) for (int j = 1;
                                                                j < FN - 1; j++)
...
static bool verify(const double *g, const double *r, size_t n, int &nfail,
                   double &mr) {
  nfail = 0;
  mr = 0;
  for (size_t i = 0; i < n; i++) {
...
struct Stg {
  ...
  void fill() {
    fill_rand(vt, se, 101);
    fill_rand(vn, (size_t)N * (nlev + 1), 102);
    fill_rand(ek, se, 103);
    fill_rand(dq, se, 104);
...
struct SoA_D {
  ...
  double *__restrict__ vt,
      *__restrict__ vn_ie, *__restrict__ z_kin_hor_e,
                               *__restrict__ ddqz_z_full_e,
                                   *__restrict__ coeff_gradekin,
                                       *__restrict__ c_lin_e,
                                           *__restrict__ f_e,
                                               *__restrict__ z_ekinh,
                                                   *__restrict__ z_w_con_c_full,
                                                       *__restrict__ zeta;
  ...
  void build(const Stg &s, bool nf_) {
    ...
    auto r2 = [&](const double *src, int M, int K) -> double * {
      ...
      double *d = nr2nf(t, M, K);
      delete[] t;
      return d;
    };
    vt = r2(s.vt, N, nlev);
    vn_ie = r2(s.vn, N, nlev + 1);
    z_kin_hor_e = r2(s.ek, N, nlev);
    ddqz_z_full_e = r2(s.dq, N, nlev);
    z_ekinh = r2(s.eh, Nc, nlev);
    z_w_con_c_full = r2(s.wc, Nc, nlev);
    zeta = r2(s.zt, Nv, nlev);
    coeff_gradekin = nr1(s.cg, N * 2);
    c_lin_e = nr1(s.cl, N * 2);
    f_e = nr1(s.fe, N);
    cell_idx = nri(s.ci, (size_t)N * 2);
    vert_idx = nri(s.vi, (size_t)N * 2);
    out = na<double>(se);
    _Pragma("omp parallel for schedule(static)") for (size_t i = 0; i < se; i++)
        out[i] = 0;
  }
  void nfree() {
    nf_(vt, se);
    nf_(vn_ie, (size_t)N * (nlev + 1));
    nf_(z_kin_hor_e, se);
    nf_(ddqz_z_full_e, se);
    nf_(z_ekinh, (size_t)Nc * nlev);
    nf_(z_w_con_c_full, (size_t)Nc * nlev);
    nf_(zeta, (size_t)Nv * nlev);
    nf_(coeff_gradekin, (size_t)N * 2);
    nf_(c_lin_e, (size_t)N * 2);
    nf_(f_e, (size_t)N);
    nf_(cell_idx, (size_t)N * 2);
    nf_(vert_idx, (size_t)N * 2);
    nf_(out, se);
  }
};
...
struct AoS_D {
  ...
  double *__restrict__ aos_e3d,
      *__restrict__ vn_ie, *__restrict__ z_kin_hor_e, *__restrict__ f_e,
                                                          *__restrict__ aos_e2d;
  ...
  void build(const Stg &s, bool nf_) {
    ...
    nlev = s.nlev;
    se = s.se;
    se3 = (size_t)NE3 * N * nlev;
    se2 = (size_t)NE2 * N;
    scn = (size_t)NCONN * N;
    scl = (size_t)ND * Nc * nlev;
    /* e3d: {vt, ddqz} */
...
static void pack_pairs(double *dst, const double *src, int N) {
  for (int je = 0; je < N; je++) {
    dst[IP(je, 0)] = src[IX(je, 0, N)];
    dst[IP(je, 1)] = src[IX(je, 1, N)];
  }
}
...
struct Grp_D {
  int N, Nc, Nv, nlev;
  size_t se, szA, szD;
  double *__restrict__ grpA,
      *__restrict__ grpD, *__restrict__ z_kin_hor_e,
                              *__restrict__ ddqz_z_full_e,
                                  *__restrict__ f_e, *__restrict__ coef,
                                                         *__restrict__ zeta;
  ...
  void build(const Stg &s, bool nf_) {
    ...
    int np = nlev + 1;
    szA = (size_t)NA * N * np;
    szD = (size_t)ND * Nc * nlev;
    double *tA = new double[szA];
    memset(tA, 0, szA * 8);
    ...
    delete[] tD;
    f_e = nr1(s.fe, N);
    /* coef[NC=4,N] = {cg0,cg1,cl0,cl1} and conn[NCONN=4,N] = {ci0,ci1,vi0,vi1}
     */
  ...
  void nfree() {
    ...
    nf_(ddqz_z_full_e, se);
    nf_(f_e, (size_t)N);
    nf_(coef, (size_t)NC * N);
    nf_(zeta, (size_t)Nv * nlev);
    nf_(conn, (size_t)NCONN * N);
    nf_(out, se);
  }
};
...
#define PS(d)                                                                  \
  ...
  const double *__restrict__ c_lin_e = (d).c_lin_e,                            \
                             *__restrict__ f_e = (d).f_e,                      \
                             *__restrict__ z_ekinh = (d).z_ekinh;              \
  const double *__restrict__ z_w_con_c_full = (d).z_w_con_c_full,              \
                             *__restrict__ zeta = (d).zeta;                    \
  const int *__restrict__ cell_idx = (d).cell_idx, *__restrict__ vert_idx =    \
                                                       (d).vert_idx;
#define PA(d)                                                                  \
  const int N = (d).N, N_c = (d).Nc, N_v = (d).Nv, nlev = (d).nlev;            \
  double *__restrict__ out = (d).out;                                          \
  const double *__restrict__ aos_e3d = (d).aos_e3d, *__restrict__ vn_ie =      \
                                                        (d).vn_ie;             \
  const double *__restrict__ z_kin_hor_e = (d).z_kin_hor_e,                    \
                             *__restrict__ f_e = (d).f_e;                      \
  const double *__restrict__ aos_e2d = (d).aos_e2d;                            \
  const int *__restrict__ aos_conn = (d).aos_conn;                             \
  const double *__restrict__ aos_cell = (d).aos_cell, *__restrict__ zeta =     \
                                                          (d).zeta;
#define PG(d)                                                                  \
  const int N = (d).N, N_c = (d).Nc, N_v = (d).Nv, nlev = (d).nlev;            \
  double *__restrict__ out = (d).out;                                          \
  ...
  const double *__restrict__ coef = (d).coef;                                  \
...
static void ks(SoA_D &d) {
  PS(d) _Pragma("omp parallel for schedule(static)") JK_O(SOA_BODY())
}
static void ks_c(SoA_D &d) {
  PS(d)
  _Pragma("omp parallel for collapse(2) schedule(static)") JK_O(SOA_BODY())
}
static void ksn(SoA_D &d) {
  PS(d) _Pragma("omp parallel for schedule(static)") JE_O(SOA_NF_BODY())
}
static void ksn_c(SoA_D &d) {
  PS(d)
  _Pragma("omp parallel for collapse(2) schedule(static)") JE_O(SOA_NF_BODY())
}
static void ka(AoS_D &d) {
  PA(d) _Pragma("omp parallel for schedule(static)") JK_O(AOS_BODY())
}
static void ka_c(AoS_D &d) {
  PA(d)
  _Pragma("omp parallel for collapse(2) schedule(static)") JK_O(AOS_BODY())
}
static void kan(AoS_D &d) {
  PA(d) _Pragma("omp parallel for schedule(static)") JE_O(AOS_NF_BODY())
}
static void kan_c(AoS_D &d) {
  PA(d)
  _Pragma("omp parallel for collapse(2) schedule(static)") JE_O(AOS_NF_BODY())
}
static void kg(Grp_D &d) {
  PG(d) _Pragma("omp parallel for schedule(static)") JK_O(GRP_BODY())
}
static void kg_c(Grp_D &d) {
  PG(d)
  _Pragma("omp parallel for collapse(2) schedule(static)") JK_O(GRP_BODY())
}
static void kgn(Grp_D &d) {
  PG(d) _Pragma("omp parallel for schedule(static)") JE_O(GRP_NF_BODY())
}
static void kgn_c(Grp_D &d) {
  PG(d)
  _Pragma("omp parallel for collapse(2) schedule(static)") JE_O(GRP_NF_BODY())
}
static void ko(AoSoA_D &d) {
  PO(d) _Pragma("omp parallel for schedule(static)") JK_O(AOSOA_BODY())
}
static void ko_c(AoSoA_D &d) {
  PO(d)
  _Pragma("omp parallel for collapse(2) schedule(static)") JK_O(AOSOA_BODY())
}
static void kon(AoSoA_D &d) {
  PO(d) _Pragma("omp parallel for schedule(static)") JE_O(AOSOA_NF_BODY())
}
static void kon_c(AoSoA_D &d) {
  PO(d)
  _Pragma("omp parallel for collapse(2) schedule(static)") JE_O(AOSOA_NF_BODY())
}
...
static void kref(SoA_D &d, double *ro) {
  ...
  const double *__restrict__ c_lin_e = d.c_lin_e, *__restrict__ f_e = d.f_e,
                             *__restrict__ z_ekinh = d.z_ekinh;
  const double *__restrict__ z_w_con_c_full = d.z_w_con_c_full,
                             *__restrict__ zeta = d.zeta;
  const int *__restrict__ cell_idx = d.cell_idx, *__restrict__ vert_idx =
...
/* ─── Benchmark runner ────────────────────────────────────────── */
static void trun(FILE *csv, const char *lay, int col, const char *dn, int nlev,
                 int N, void (*fn)(), double *outp, const double *ref,
                 size_t se, bool isnf) {
  fn();
  int nfail;
  double mr;
  if (!isnf)
    verify(outp, ref, se, nfail, mr);
  else {
...
int main(int argc, char *argv[]) {
  ...
  printf("ddt_vn CPU sweep  N=%d thr=%d NRUNS=%d VW=%d NA=%d ND=%d NE3=%d "
         "NE2=%d\n",
  ...
  for (int nlev : nlevs) {
    ...
    for (int di = 0; di < n_dists; di++) {
      ...
      _Pragma("omp parallel for schedule(static)") for (size_t i = 0; i < se;
                                                        i++) ref[i] = 0;
      kref(sj, ref);
  ...
  printf(
      "  %zu nlevs × %d dists × 8 layouts × 2 collapse × %d reps = %zu rows\n",
      nlevs.size(), n_dists, NRUNS, nlevs.size() * n_dists * 8 * 2 * NRUNS);
  return 0;
}