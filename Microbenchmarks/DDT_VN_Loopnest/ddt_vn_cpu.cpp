...
template<typename T>static T*na(size_t n){
    if(n==0)n=1; /* mmap rejects length 0 */
    size_t bytes=n*sizeof(T);
    void*p=mmap(0,bytes,PROT_READ|PROT_WRITE,MAP_PRIVATE|MAP_ANONYMOUS,-1,0);
    if(p==MAP_FAILED){perror("mmap");fprintf(stderr,"  requested %zu bytes\n",bytes);exit(1);}
    madvise(p,bytes,MADV_HUGEPAGE);
    return(T*)p;}
template<typename T>static void nf_(T*p,size_t n){if(n==0)n=1;munmap(p,n*sizeof(T));}
...
static void flush(){double*A=fb0,*B=fb1;for(int s=0;s<FS;s++){_Pragma("omp parallel for schedule(static)")for(int i=1;i<FN-1;i++)for(int j=1;j<FN-1;j++)B[i*FN+j]=0.25*(A[(i-1)*FN+j]+A[(i+1)*FN+j]+A[i*FN+(j-1)]+A[i*FN+(j+1)]);std::swap(A,B);}}
...
struct SoA_D{int N,Nc,Nv,nlev;size_t se;
    double*__restrict__ vt,*__restrict__ vn_ie,*__restrict__ z_kin_hor_e,*__restrict__ ddqz_z_full_e,*__restrict__ coeff_gradekin,*__restrict__ c_lin_e,*__restrict__ f_e,*__restrict__ z_ekinh,*__restrict__ z_w_con_c_full,*__restrict__ zeta;int*__restrict__ cell_idx,*__restrict__ vert_idx;double*__restrict__ out;
...
struct AoS_D{int N,Nc,Nv,nlev;size_t se,se3,se2,scn,scl;
    double*__restrict__ aos_e3d,*__restrict__ vn_ie,*__restrict__ z_kin_hor_e,*__restrict__ f_e,*__restrict__ aos_e2d;int*__restrict__ aos_conn;double*__restrict__ aos_cell,*__restrict__ zeta,*__restrict__ out;
    void build(const Stg&s,bool nf_){N=s.N;Nc=s.Nc;Nv=s.Nv;nlev=s.nlev;se=s.se;
...
struct Grp_D{int N,Nc,Nv,nlev;size_t se,szA,szD;
    double*__restrict__ grpA,*__restrict__ grpD,*__restrict__ z_kin_hor_e,*__restrict__ ddqz_z_full_e,*__restrict__ f_e,*__restrict__ coef,*__restrict__ zeta;int*__restrict__ conn;double*__restrict__ out;
    void build(const Stg&s,bool nf_){N=s.N;Nc=s.Nc;Nv=s.Nv;nlev=s.nlev;se=s.se;int np=nlev+1;
        ...
        double*tc=new double[(size_t)NC*N];for(int je=0;je<N;je++){tc[IC(C_CG0,je)]=s.cg[IX(je,0,N)];tc[IC(C_CG1,je)]=s.cg[IX(je,1,N)];tc[IC(C_CL0,je)]=s.cl[IX(je,0,N)];tc[IC(C_CL1,je)]=s.cl[IX(je,1,N)];}
        coef=nr1(tc,(size_t)NC*N);delete[]tc;
        int*tn=new int[(size_t)NCONN*N];for(int je=0;je<N;je++){tn[ICN(CN_CI0,je)]=s.ci[IX(je,0,N)];tn[ICN(CN_CI1,je)]=s.ci[IX(je,1,N)];tn[ICN(CN_VI0,je)]=s.vi[IX(je,0,N)];tn[ICN(CN_VI1,je)]=s.vi[IX(je,1,N)];}
    ...
    void nfree(){nf_(grpA,szA);nf_(grpD,szD);nf_(z_kin_hor_e,se);nf_(ddqz_z_full_e,se);nf_(f_e,(size_t)N);
        nf_(coef,(size_t)NC*N);nf_(zeta,(size_t)Nv*nlev);
        nf_(conn,(size_t)NCONN*N);nf_(out,se);}};
