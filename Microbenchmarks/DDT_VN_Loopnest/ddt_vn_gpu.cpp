...
struct GrpH {
  ...
  void from(const SoAH &s) {
    ...
    for (int je = 0; je < N; je++) {
      coef[IC(C_CG0, je)] = s.cg[IX(je, 0, N)];
      coef[IC(C_CG1, je)] = s.cg[IX(je, 1, N)];
      coef[IC(C_CL0, je)] = s.cl[IX(je, 0, N)];
      coef[IC(C_CL1, je)] = s.cl[IX(je, 1, N)];
    }
    conn = new int[(size_t)NCONN * N];
    for (int je = 0; je < N; je++) {
      conn[ICN(CN_CI0, je)] = s.ci[IX(je, 0, N)];
      conn[ICN(CN_CI1, je)] = s.ci[IX(je, 1, N)];
      conn[ICN(CN_VI0, je)] = s.vi[IX(je, 0, N)];
      conn[ICN(CN_VI1, je)] = s.vi[IX(je, 1, N)];
    }
    out = new double[se];
    memset(out, 0, se * 8);
  }
...
struct DG {
  double *gA, *gD, *ek, *dq, *fe, *coef, *zt, *out;
  int *conn;
};
struct DO_ {
  double *gA, *gD, *ek, *dq, *fe, *coef, *zt, *out;
...
kg(double *__restrict__ out, const double *__restrict__ grpA,
   ...
   const double *__restrict__ coef, const double *__restrict__ zeta,
   const int *__restrict__ conn, int N, int N_c, int N_v, int nlev) {
  KL(TX, TY, GRP_BODY())
}
...
static void adg(DG &d, const GrpH &h) {
  ...
  AL(d.dq, h.se * 8);
  AL(d.fe, N * 8);
  AL(d.coef, (size_t)NC * N * 8);
  AL(d.zt, (size_t)Nv * nl * 8);
  AL(d.conn, (size_t)NCONN * N * 4);
  AL(d.out, h.se * 8);
  ...
  GC(d.dq, h.dq, h.se * 8);
  GC(d.fe, h.fe, N * 8);
  GC(d.coef, h.coef, (size_t)NC * N * 8);
    ...
    for (int ci = 0; ci < NC_; ci++) {
      ...
      BENCH("grp",
            DT(kg, c.tx, c.ty, gr, bk, dg.out, dg.gA, dg.gD, dg.ek, dg.dq,
               dg.fe, dg.coef, dg.zt, dg.conn, N, Nc, Nv, nl),
            0)
      BENCH("aosoa",
            DTV(ko, 32, c.tx, c.ty, gr, bk, d32.out, d32.gA, d32.gD, d32.ek,
                d32.dq, d32.fe, d32.coef, d32.zt, d32.conn, N, Nc, Nv, nl),
            32)
      BENCH("aosoa",
            DTV(ko, 64, c.tx, c.ty, gr, bk, d64.out, d64.gA, d64.gD, d64.ek,
                d64.dq, d64.fe, d64.coef, d64.zt, d64.conn, N, Nc, Nv, nl),
            64)
      fflush(csv);
      printf(" done\n");
    }
