 ...
 *   index(f, je, jk) = jk * tiles_N * F * V  +  (je / V) * F * V  +  f * V  +
 * (je % V) where tiles_N = ceil(N / V)
 *
 * Layout for a 2D array [F, N]:
 *   index(f, je) = (je / V) * F * V  +  f * V  +  (je % V)
 */
template <int V, int F> HD int IAoSoA_3D(int f, int je, int jk, int N) {
  int tiles = (N + V - 1) / V;
  return jk * tiles * F * V + (je / V) * F * V + f * V + (je % V);
}
template <int V, int F> HD int IAoSoA_2D(int f, int je) {
...
template <int V> size_t aosoa_grpA_size(int N, int nlevp1) {
  return (size_t)((N + V - 1) / V) * NA * V * nlevp1;
}
template <int V> size_t aosoa_grpC_size(int N) {
  return (size_t)((N + V - 1) / V) * NC * V;
}
template <int V> size_t aosoa_grpD_size(int Nc, int nlev) {
...
#define SOA_BODY()                                                             \
  ...
  out[c2] =                                                                    \
      -(z_kin_hor_e[c2] * (cg0 - cg1) + cg1 * z_ekinh[I2(ci1, jk, N_c)] -      \
        cg0 * z_ekinh[I2(ci0, jk, N_c)] +                                      \
        vt[c2] * (f_e[je] +                                                    \
                  0.5 * (zeta[I2(vi0, jk, N_v)] + zeta[I2(vi1, jk, N_v)])) +   \
...
#define GRP_BODY()                                                             \
  ...
  out[I2(je, jk, N)] =                                                         \
      -(ekin_e * (cg0 - cg1) + cg1 * ekinh1 - cg0 * ekinh0 +                   \
        vt_e *                                                                 \
            (fe_e + 0.5 * (zeta[I2(vi0, jk, N_v)] + zeta[I2(vi1, jk, N_v)])) + \
        (cl0 * wcon0 + cl1 * wcon1) * (vnie_k - vnie_k1) /                     \
            ddqz_z_full_e[I2(je, jk, N)]);
...
static void gen_connectivity(int *L, int N, int N_target, CellDist dist,
                             std::mt19937 &rng) {
  switch (dist) {
  case UNIFORM: {
  ...
  case NORMAL1: {
    ...
    for (int i = 0; i < N; i++) {
      L[i * 2] =
          ((i + 1 + (int)std::round(nd(rng))) % N_target + N_target) % N_target;
      L[i * 2 + 1] =
          ((i - 1 + (int)std::round(nd(rng))) % N_target + N_target) % N_target;
    }
    break;
...
static void flush_destroy() { GPU_FREE(d_flush_buf); }
...
struct SoAData {
  ...
  void alloc(int N_, int Nc, int Nv, int nlev_) {
    ...
    sz_v = (size_t)Nv * nlev;
    vt = new double[sz_e];
    vn_ie = new double[(size_t)N * (nlev + 1)];
  ...
  void fill() {
    fill_rand(vt, sz_e, 101);
    fill_rand(vn_ie, (size_t)N * (nlev + 1), 102);
    fill_rand(z_kin_hor_e, sz_e, 103);
    fill_rand(ddqz_z_full_e, sz_e, 104);
    fill_rand(coeff_gradekin, N * 2, 105);
    fill_rand(c_lin_e, N * 2, 106);
    fill_rand(f_e, N, 107);
    fill_rand(z_ekinh, sz_c, 108);
    fill_rand(z_w_con_c_full, sz_c, 109);
    fill_rand(zeta, sz_v, 110);
    memset(out, 0, sz_e * sizeof(double));
    for (size_t i = 0; i < sz_e; i++)
      if (std::abs(ddqz_z_full_e[i]) < 1e-10)
        ddqz_z_full_e[i] = 1.0;
  }
  void free_all() {
    delete[] vt;
    delete[] vn_ie;
    delete[] z_kin_hor_e;
    delete[] ddqz_z_full_e;
    delete[] coeff_gradekin;
    delete[] c_lin_e;
    delete[] f_e;
    delete[] z_ekinh;
...
template <int V> struct AoSoAData {
  int N, N_c, N_v, nlev;
  size_t sz_e;
  double *grpA, *grpC, *grpD;
  double *ddqz_z_full_e, *coeff_gradekin, *c_lin_e, *zeta;
  ...
  void from_soa(const SoAData &s) {
    ...
    grpA = new double[szA];
    memset(grpA, 0, szA * sizeof(double));
    grpC = new double[szC];
    memset(grpC, 0, szC * sizeof(double));
    grpD = new double[szD];
    memset(grpD, 0, szD * sizeof(double));
    for (int jk = 0; jk < nlev; jk++)
      for (int je = 0; je < N; je++) {
    ...
    for (int jk = 0; jk < nlev; jk++)
      for (int ic = 0; ic < N_c; ic++) {
        ...
        grpD[ID_aosoa<V>(D_WCON, ic, jk, N_c)] =
            s.z_w_con_c_full[I2(ic, jk, N_c)];
      }
    ddqz_z_full_e = new double[sz_e];
    coeff_gradekin = new double[N * 2];
    c_lin_e = new double[N * 2];
...
__global__ void
kern_soa(double *__restrict__ out, const double *__restrict__ vt,
         const double *__restrict__ vn_ie,
         const double *__restrict__ z_kin_hor_e,
         ...
         const double *__restrict__ z_w_con_c_full,
         const double *__restrict__ zeta, const int *__restrict__ cell_idx,
         const int *__restrict__ vert_idx, int N, int N_c, int N_v, int nlev) {
...
template <int TX, int TY>
__global__ void
kern_grp(double *__restrict__ out, const double *__restrict__ grpA,
         const double *__restrict__ grpC, const double *__restrict__ grpD,
         const double *__restrict__ ddqz_z_full_e,
         const double *__restrict__ coeff_gradekin,
...
__global__ void
kern_aosoa(double *__restrict__ out, const double *__restrict__ grpA,
           const double *__restrict__ grpC, const double *__restrict__ grpD,
           const double *__restrict__ ddqz_z_full_e,
           const double *__restrict__ coeff_gradekin,
           const double *__restrict__ c_lin_e, const double *__restrict__ zeta,
...
/*  Verification                                                     */
/* ================================================================ */
static bool verify(const double *got, const double *ref, size_t n, int &nfail,
                   double &maxrel) {
  nfail = 0;
  maxrel = 0;
  for (size_t i = 0; i < n; i++) {
...
#define DISPATCH_TXTY(KERN, tx, ty, grid, block, ...)                          \
  do {                                                                         \
    if ((tx) == 1 && (ty) == 1)                                                \
      KERN<1, 1><<<grid, block>>>(__VA_ARGS__);                                \
    else if ((tx) == 2 && (ty) == 1)                                           \
      KERN<2, 1><<<grid, block>>>(__VA_ARGS__);                                \
    else if ((tx) == 4 && (ty) == 1)                                           \
      KERN<4, 1><<<grid, block>>>(__VA_ARGS__);                                \
    else if ((tx) == 1 && (ty) == 2)                                           \
      KERN<1, 2><<<grid, block>>>(__VA_ARGS__);                                \
    else if ((tx) == 1 && (ty) == 4)                                           \
      KERN<1, 4><<<grid, block>>>(__VA_ARGS__);                                \
    else if ((tx) == 2 && (ty) == 2)                                           \
      KERN<2, 2><<<grid, block>>>(__VA_ARGS__);                                \
    else if ((tx) == 4 && (ty) == 2)                                           \
      KERN<4, 2><<<grid, block>>>(__VA_ARGS__);                                \
    else if ((tx) == 2 && (ty) == 4)                                           \
      KERN<2, 4><<<grid, block>>>(__VA_ARGS__);                                \
    else {                                                                     \
      fprintf(stderr, "Unsupported TX=%d TY=%d\n", tx, ty);                    \
      exit(1);                                                                 \
    }                                                                          \
  } while (0)

/* Same but with an extra template parameter V for AoSoA */
#define DISPATCH_V_TXTY(KERN, V, tx, ty, grid, block, ...)                     \
  do {                                                                         \
    if ((tx) == 1 && (ty) == 1)                                                \
      KERN<V, 1, 1><<<grid, block>>>(__VA_ARGS__);                             \
    else if ((tx) == 2 && (ty) == 1)                                           \
      KERN<V, 2, 1><<<grid, block>>>(__VA_ARGS__);                             \
    else if ((tx) == 4 && (ty) == 1)                                           \
      KERN<V, 4, 1><<<grid, block>>>(__VA_ARGS__);                             \
    else if ((tx) == 1 && (ty) == 2)                                           \
      KERN<V, 1, 2><<<grid, block>>>(__VA_ARGS__);                             \
    else if ((tx) == 1 && (ty) == 4)                                           \
      KERN<V, 1, 4><<<grid, block>>>(__VA_ARGS__);                             \
    else if ((tx) == 2 && (ty) == 2)                                           \
      KERN<V, 2, 2><<<grid, block>>>(__VA_ARGS__);                             \
    else if ((tx) == 4 && (ty) == 2)                                           \
      KERN<V, 4, 2><<<grid, block>>>(__VA_ARGS__);                             \
    else if ((tx) == 2 && (ty) == 4)                                           \
      KERN<V, 2, 4><<<grid, block>>>(__VA_ARGS__);                             \
    else {                                                                     \
      fprintf(stderr, "Unsupported TX=%d TY=%d\n", tx, ty);                    \
      exit(1);                                                                 \
    }                                                                          \
...
static void alloc_dev_soa(DevSoA &d, const SoAData &h) {
  ...
  GPU_CHECK(GPU_MEMCPY(d.vn_ie, h.vn_ie,
                       (size_t)N * (nlev + 1) * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.ekin, h.z_kin_hor_e, h.sz_e * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.ddqz, h.ddqz_z_full_e, h.sz_e * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.cg, h.coeff_gradekin, N * 2 * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(
      GPU_MEMCPY(d.cl, h.c_lin_e, N * 2 * sizeof(double), GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.fe, h.f_e, N * sizeof(double), GPU_MEMCPY_H2D));
  GPU_CHECK(
      GPU_MEMCPY(d.ekinh, h.z_ekinh, h.sz_c * sizeof(double), GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.wcon, h.z_w_con_c_full, h.sz_c * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(
      GPU_MEMCPY(d.zeta, h.zeta, h.sz_v * sizeof(double), GPU_MEMCPY_H2D));
  GPU_CHECK(
      GPU_MEMCPY(d.cidx, h.cell_idx, N * 2 * sizeof(int), GPU_MEMCPY_H2D));
  GPU_CHECK(
      GPU_MEMCPY(d.vidx, h.vert_idx, N * 2 * sizeof(int), GPU_MEMCPY_H2D));
}
...
static void alloc_dev_grp(DevGrp &d, const GrpData &h) {
  ...
  GPU_CHECK(GPU_MEMCPY(d.grpA, h.grpA,
                       (size_t)NA * N * (nlev + 1) * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.grpC, h.grpC, (size_t)NC * N * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.grpD, h.grpD, (size_t)ND * Nc * nlev * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.ddqz, h.ddqz_z_full_e, h.sz_e * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.cg, h.coeff_gradekin, N * 2 * sizeof(double),
  ...
  GPU_CHECK(
      GPU_MEMCPY(d.cl, h.c_lin_e, N * 2 * sizeof(double), GPU_MEMCPY_H2D));
  GPU_CHECK(GPU_MEMCPY(d.zeta, h.zeta, (size_t)Nv * nlev * sizeof(double),
                       GPU_MEMCPY_H2D));
  GPU_CHECK(
      GPU_MEMCPY(d.cidx, h.cell_idx, N * 2 * sizeof(int), GPU_MEMCPY_H2D));
  GPU_CHECK(
...
/*  Benchmark runner                                                 */
/* ================================================================ */
static void bench_soa(FILE *csv, const char *dist_name, const KernelConfig &cfg,
  ...
  for (int w = 0; w < WARMUP; w++) {
    DISPATCH_TXTY(kern_soa, cfg.tx, cfg.ty, grid, block, d.out, d.vt, d.vn_ie,
                  d.ekin, d.ddqz, d.cg, d.cl, d.fe, d.ekinh, d.wcon, d.zeta,
  ...
  for (int r = 0; r < NRUNS; r++) {
    ...
    float ms;
    GPU_CHECK(GPU_EVENT_ELAPSED(&ms, ev0, ev1));
    fprintf(csv, "soa,%s,%d,%d,%d,%d,0,%d,%.6f\n", dist_name, cfg.bx, cfg.by,
            cfg.tx, cfg.ty, r, (double)ms);
  }
}

static void bench_grp(FILE *csv, const char *dist_name, const KernelConfig &cfg,
  ...
  for (int w = 0; w < WARMUP; w++) {
    DISPATCH_TXTY(kern_grp, cfg.tx, cfg.ty, grid, block, d.out, d.grpA, d.grpC,
                  d.grpD, d.ddqz, d.cg, d.cl, d.zeta, d.cidx, d.vidx, N, Nc, Nv,
                  nlev);
  }
  GPU_CHECK(GPU_SYNC());
...
static void bench_aosoa(FILE *csv, const char *dist_name,
                        const KernelConfig &cfg, DevAoSoA &d, int N, int Nc,
                        int Nv, int nlev, GPU_EVENT_T ev0, GPU_EVENT_T ev1) {
  dim3 block(cfg.bx, cfg.by);
  dim3 grid((N + block.x * cfg.tx - 1) / (block.x * cfg.tx),
            (nlev + block.y * cfg.ty - 1) / (block.y * cfg.ty));

  for (int w = 0; w < WARMUP; w++) {
    DISPATCH_V_TXTY(kern_aosoa, V, cfg.tx, cfg.ty, grid, block, d.out, d.grpA,
                    d.grpC, d.grpD, d.ddqz, d.cg, d.cl, d.zeta, d.cidx, d.vidx,
                    N, Nc, Nv, nlev);
  }
  GPU_CHECK(GPU_SYNC());
...
int main(int argc, char *argv[]) {
  ...
  for (int di = 0; di < 4; di++) {
    ...
    /* ---- Allocate device ---- */
    DevSoA dev_soa;
    alloc_dev_soa(dev_soa, soa);
    DevGrp dev_grp;
    alloc_dev_grp(dev_grp, grp);
    DevAoSoA dev_a32;
    alloc_dev_aosoa<32>(dev_a32, aosoa32);
    DevAoSoA dev_a64;
    alloc_dev_aosoa<64>(dev_a64, aosoa64);
    ...
    /* ---- Verify all layouts (baseline config) ---- */
    {
      ...
      GPU_CHECK(GPU_SYNC());
      GPU_CHECK(GPU_MEMCPY(h_out, dev_soa.out, soa.sz_e * sizeof(double),
                           GPU_MEMCPY_D2H));
      verify(h_out, h_ref, soa.sz_e, nf, mr);
      printf("  SoA    verify: %s  max_rel=%.2e\n", nf ? "FAIL" : "OK", mr);

      kern_grp<1, 1><<<grid, block>>>(dev_grp.out, dev_grp.grpA, dev_grp.grpC,
                                      dev_grp.grpD, dev_grp.ddqz, dev_grp.cg,
                                      dev_grp.cl, dev_grp.zeta, dev_grp.cidx,
                                      dev_grp.vidx, N, N_c, N_v, nlev);
      GPU_CHECK(GPU_SYNC());
      GPU_CHECK(GPU_MEMCPY(h_out, dev_grp.out, soa.sz_e * sizeof(double),
                           GPU_MEMCPY_D2H));
      verify(h_out, h_ref, soa.sz_e, nf, mr);
      printf("  AoS    verify: %s  max_rel=%.2e\n", nf ? "FAIL" : "OK", mr);

      kern_aosoa<32, 1, 1><<<grid, block>>>(
          dev_a32.out, dev_a32.grpA, dev_a32.grpC, dev_a32.grpD, dev_a32.ddqz,
          dev_a32.cg, dev_a32.cl, dev_a32.zeta, dev_a32.cidx, dev_a32.vidx, N,
          N_c, N_v, nlev);
      GPU_CHECK(GPU_SYNC());
      GPU_CHECK(GPU_MEMCPY(h_out, dev_a32.out, soa.sz_e * sizeof(double),
                           GPU_MEMCPY_D2H));
      verify(h_out, h_ref, soa.sz_e, nf, mr);
      printf("  AoSoA32 verify: %s  max_rel=%.2e\n", nf ? "FAIL" : "OK", mr);

      kern_aosoa<64, 1, 1><<<grid, block>>>(
          dev_a64.out, dev_a64.grpA, dev_a64.grpC, dev_a64.grpD, dev_a64.ddqz,
          dev_a64.cg, dev_a64.cl, dev_a64.zeta, dev_a64.cidx, dev_a64.vidx, N,
          N_c, N_v, nlev);
      GPU_CHECK(GPU_SYNC());
      GPU_CHECK(GPU_MEMCPY(h_out, dev_a64.out, soa.sz_e * sizeof(double),
                           GPU_MEMCPY_D2H));
      verify(h_out, h_ref, soa.sz_e, nf, mr);
    ...
    for (int ci = 0; ci < NCONFIGS; ci++) {
      ...
      printf("  config %2d/%d: block=(%3d,%d) coarsen=(%d,%d) ...", ci + 1,
             NCONFIGS, cfg.bx, cfg.by, cfg.tx, cfg.ty);
      fflush(stdout);
      ...
      bench_aosoa<32>(csv, dist_names[di], cfg, dev_a32, N, N_c, N_v, nlev, ev0,
                      ev1);
      bench_aosoa<64>(csv, dist_names[di], cfg, dev_a64, N, N_c, N_v, nlev, ev0,
                      ev1);
  ...
  printf("  Total rows: %d dist × %d configs × 4 layouts × %d reps = %d\n", 4,
         NCONFIGS, NRUNS, 4 * NCONFIGS * 4 * NRUNS);
  return 0;
}