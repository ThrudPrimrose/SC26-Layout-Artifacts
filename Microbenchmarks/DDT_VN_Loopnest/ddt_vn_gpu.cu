...
int main(int argc,char*argv[]){
    ...
    for(int di=0;di<4;di++){CellDist dist=(CellDist)di;printf("\n=== dist=%s ===\n",dist_names[di]);
        SoAH soa;soa.alloc(N,Nc,Nv,nl);soa.fill();gen_conn(soa.ci,N,Nc,dist,rng);gen_conn(soa.vi,N,Nv,dist,rng);
