 ...
 * nvcc  -O3 -std=c++17 -arch=sm_90 bench_ddt_vn_gpu_sweep.cu -o bench
 */
#include <chrono>
#include <cmath>
#include <cstdint>
...
#define GPU_CHECK(c) do{hipError_t e=(c);if(e!=hipSuccess){fprintf(stderr,"HIP %s:%d: %s\n",__FILE__,__LINE__,hipGetErrorString(e));exit(1);}}while(0)
#define GM hipMalloc
#define GF hipFree
...
#define GRP_BODY() {\
    int ci0=conn[ICN(CN_CI0,je)],ci1=conn[ICN(CN_CI1,je)],vi0=conn[ICN(CN_VI0,je)],vi1=conn[ICN(CN_VI1,je)];\
    double cg0=coef[IC(C_CG0,je)],cg1=coef[IC(C_CG1,je)],cl0=coef[IC(C_CL0,je)],cl1=coef[IC(C_CL1,je)];\
...
static void gen_conn(int*L,int N,int Nt,CellDist d,std::mt19937&rng){switch(d){case UNIFORM:{std::uniform_int_distribution<int>u(0,Nt-1);for(int i=0;i<N;i++){L[i*2]=u(rng);L[i*2+1]=u(rng);}break;}case NORMAL1:{std::normal_distribution<double>nd(0,1);for(int i=0;i<N;i++){L[i*2]=((i+1+(int)std::round(nd(rng)))%Nt+Nt)%Nt;L[i*2+1]=((i-1+(int)std::round(nd(rng)))%Nt+Nt)%Nt;}break;}case NORMAL4:{std::normal_distribution<double>nd(0,2);for(int i=0;i<N;i++){L[i*2]=((i+1+(int)std::round(nd(rng)))%Nt+Nt)%Nt;L[i*2+1]=((i-1+(int)std::round(nd(rng)))%Nt+Nt)%Nt;}break;}case SEQUENTIAL:for(int i=0;i<N;i++){L[i*2]=(i+1)%Nt;L[i*2+1]=(i+1)%Nt;}break;case EXACT:break;}}
...
struct GrpH{int N,Nc,Nv,nl;size_t se;double*gA,*gD,*ek,*dq,*fe,*coef,*zt;int*conn;double*out;
    void from(const SoAH&s){N=s.N;Nc=s.Nc;Nv=s.Nv;nl=s.nl;se=s.se;int np=nl+1;
        ...
        coef=new double[(size_t)NC*N];for(int je=0;je<N;je++){coef[IC(C_CG0,je)]=s.cg[IX(je,0,N)];coef[IC(C_CG1,je)]=s.cg[IX(je,1,N)];coef[IC(C_CL0,je)]=s.cl[IX(je,0,N)];coef[IC(C_CL1,je)]=s.cl[IX(je,1,N)];}
        conn=new int[(size_t)NCONN*N];for(int je=0;je<N;je++){conn[ICN(CN_CI0,je)]=s.ci[IX(je,0,N)];conn[ICN(CN_CI1,je)]=s.ci[IX(je,1,N)];conn[ICN(CN_VI0,je)]=s.vi[IX(je,0,N)];conn[ICN(CN_VI1,je)]=s.vi[IX(je,1,N)];}
...
template<int V>struct AoSoAH{int N,Nc,Nv,nl;size_t se,sA,sD;double*gA,*gD,*ek,*dq,*fe,*coef,*zt;int*conn;double*out;
...
template<int TX,int TY>__global__ void kg(double*__restrict__ out,const double*__restrict__ grpA,const double*__restrict__ grpD,const double*__restrict__ z_kin_hor_e,const double*__restrict__ ddqz_z_full_e,const double*__restrict__ f_e,const double*__restrict__ coef,const double*__restrict__ zeta,const int*__restrict__ conn,int N,int N_c,int N_v,int nlev){KL(TX,TY,GRP_BODY())}
template<int V,int TX,int TY>__global__ void ko(double*__restrict__ out,const double*__restrict__ grpA,const double*__restrict__ grpD,const double*__restrict__ z_kin_hor_e,const double*__restrict__ ddqz_z_full_e,const double*__restrict__ f_e,const double*__restrict__ coef,const double*__restrict__ zeta,const int*__restrict__ conn,int N,int N_c,int N_v,int nlev){KL(TX,TY,AOSOA_BODY(V))}
...
static void adg(DG&d,const GrpH&h){int N=h.N,Nc=h.Nc,Nv=h.Nv,nl=h.nl;AL(d.gA,(size_t)NA*N*(nl+1)*8);AL(d.gD,(size_t)ND*Nc*nl*8);AL(d.ek,h.se*8);AL(d.dq,h.se*8);AL(d.fe,N*8);AL(d.coef,(size_t)NC*N*8);AL(d.zt,(size_t)Nv*nl*8);AL(d.conn,(size_t)NCONN*N*4);AL(d.out,h.se*8);GC(d.gA,h.gA,(size_t)NA*N*(nl+1)*8);GC(d.gD,h.gD,(size_t)ND*Nc*nl*8);GC(d.ek,h.ek,h.se*8);GC(d.dq,h.dq,h.se*8);GC(d.fe,h.fe,N*8);GC(d.coef,h.coef,(size_t)NC*N*8);GC(d.zt,h.zt,(size_t)Nv*nl*8);GC(d.conn,h.conn,(size_t)NCONN*N*4);}
static void fdg(DG&d){GF(d.gA);GF(d.gD);GF(d.ek);GF(d.dq);GF(d.fe);GF(d.coef);GF(d.zt);GF(d.conn);GF(d.out);}
template<int V>static void ado(DO_&d,const AoSoAH<V>&h){int N=h.N,Nv=h.Nv,nl=h.nl;d.sA=h.sA;d.sD=h.sD;AL(d.gA,h.sA*8);AL(d.gD,h.sD*8);AL(d.ek,h.se*8);AL(d.dq,h.se*8);AL(d.fe,N*8);AL(d.coef,(size_t)NC*N*8);AL(d.zt,(size_t)Nv*nl*8);AL(d.conn,(size_t)NCONN*N*4);AL(d.out,h.se*8);GC(d.gA,h.gA,h.sA*8);GC(d.gD,h.gD,h.sD*8);GC(d.ek,h.ek,h.se*8);GC(d.dq,h.dq,h.se*8);GC(d.fe,h.fe,N*8);GC(d.coef,h.coef,(size_t)NC*N*8);GC(d.zt,h.zt,(size_t)Nv*nl*8);GC(d.conn,h.conn,(size_t)NCONN*N*4);}
...
int main(int argc,char*argv[]){
    ...
    for(int di=0;di<n_dists;di++){printf("\n=== dist=%s ===\n",dist_names[di]);
        ...
        {dim3 bk(64,1),gr((N+63)/64,nl);double*ho=new double[soa.se];int nf;double mr;
            ...
            kg<1,1><<<gr,bk>>>(dg.out,dg.gA,dg.gD,dg.ek,dg.dq,dg.fe,dg.coef,dg.zt,dg.conn,N,Nc,Nv,nl);GPU_CHECK(GS());GCD(ho,dg.out,soa.se*8);verify(ho,hr,soa.se,nf,mr);printf("  Grp    %s %.2e\n",nf?"FAIL":"OK",mr);
            ko<32,1,1><<<gr,bk>>>(d32.out,d32.gA,d32.gD,d32.ek,d32.dq,d32.fe,d32.coef,d32.zt,d32.conn,N,Nc,Nv,nl);GPU_CHECK(GS());GCD(ho,d32.out,soa.se*8);verify(ho,hr,soa.se,nf,mr);printf("  AoSoA32 %s %.2e\n",nf?"FAIL":"OK",mr);
            ko<64,1,1><<<gr,bk>>>(d64.out,d64.gA,d64.gD,d64.ek,d64.dq,d64.fe,d64.coef,d64.zt,d64.conn,N,Nc,Nv,nl);GPU_CHECK(GS());GCD(ho,d64.out,soa.se*8);verify(ho,hr,soa.se,nf,mr);printf("  AoSoA64 %s %.2e\n",nf?"FAIL":"OK",mr);
            delete[]ho;}
