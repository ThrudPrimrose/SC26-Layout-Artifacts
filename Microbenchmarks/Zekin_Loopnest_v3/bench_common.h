...
static void gen_cell_idx_logical(int *L, int N, CellDist dist, std::mt19937 &rng) {
  switch (dist) {
  case UNIFORM: {
...
enum SchedKind { SCHED_JK_OUTER=0, SCHED_JE_OUTER=1, SCHED_COLLAPSE2=2, SCHED_NUMA4=3 };
...
static void rearrange_idx(int V, int *dst, const int *logical, int N) {
  switch(V){case 1:layout_idx<1>(dst,logical,N);break;case 2:layout_idx<2>(dst,logical,N);break;
            case 3:layout_idx<3>(dst,logical,N);break;case 4:layout_idx<4>(dst,logical,N);break;}
}
...
#pragma omp parallel for collapse(2) schedule(static)
    for (int jk = 0; jk < nlev; jk++)
      for (int je = 0; je < N; je++) { int i=IC<V>(je,jk,N,nlev); dst[i]=src[i]; }
    break;
  case SCHED_JE_OUTER:
#pragma omp parallel for schedule(static)
    for (int je = 0; je < N; je++)
      for (int jk = 0; jk < nlev; jk++) { int i=IC<V>(je,jk,N,nlev); dst[i]=src[i]; }
    break;
  case SCHED_JK_OUTER:
#pragma omp parallel for schedule(static)
    for (int jk = 0; jk < nlev; jk++)
  ...
  case SCHED_NUMA4: {
    ...
    #pragma omp parallel num_threads(ND)
    {
      ...
      int je0 = d * chunk;
      int je1 = (d == ND-1) ? N : je0 + chunk;
      #pragma omp parallel for schedule(static) num_threads(tpd)
...
template <int V>
static int *redistribute_idx(const int *src, int N, SchedKind sched) {
...
#pragma omp parallel for collapse(2) schedule(static)
    for (int jb = 0; jb < nblocks; jb++)
      for (int jk = 0; jk < nlev; jk++)
        for (int jl = 0; jl < B; jl++) { int i=IC_blocked(jb*B+jl,jk,B,nlev); dst[i]=src[i]; }
    break;
  default:
#pragma omp parallel for schedule(static)
    for (int jb = 0; jb < nblocks; jb++)
...
struct BenchData {
  ...
  double *h_vn_ie=nullptr, *h_w=nullptr, *h_z_vt_ie=nullptr, *h_z_w_v=nullptr, *h_out=nullptr;
  ...
  void alloc(int N_, int nlev_) {
    N=N_; nlev=nlev_; sz2d=(size_t)N*nlev;
    src_vn_ie=new double[sz2d]; src_w=new double[sz2d];
    src_z_vt_ie=new double[sz2d]; src_z_w_v=new double[sz2d];
    inv_dual=new double[N]; inv_primal=new double[N]; tangent_o=new double[N];
    h_vn_ie=new double[sz2d]; h_w=new double[sz2d];
    h_z_vt_ie=new double[sz2d]; h_z_w_v=new double[sz2d];
    h_out=new double[sz2d];
    h_cidx=new int[N*2]; h_vidx=new int[N*2];
  }
  void fill(int nlev_) {
    fill_xor(src_vn_ie,sz2d,100+nlev_); fill_xor(src_w,sz2d,200+nlev_);
    fill_xor(src_z_vt_ie,sz2d,300+nlev_); fill_xor(src_z_w_v,sz2d,400+nlev_);
    fill_xor(inv_dual,N,500); fill_xor(inv_primal,N,600); fill_xor(tangent_o,N,700);
  }
  void set_variant(int V, const int *cell_logical, const int *vert_logical,
                   SchedKind sched = SCHED_JK_OUTER) {
    cur_V=V; cur_B=0;
    rearrange_2d(V,h_vn_ie,src_vn_ie,N,nlev); rearrange_2d(V,h_w,src_w,N,nlev);
    rearrange_2d(V,h_z_vt_ie,src_z_vt_ie,N,nlev); rearrange_2d(V,h_z_w_v,src_z_w_v,N,nlev);
    rearrange_idx(V,h_cidx,cell_logical,N); rearrange_idx(V,h_vidx,vert_logical,N);
#ifndef __CUDACC__
  ...
  void set_variant_blocked(int B, const int *cell_logical, const int *vert_logical,
                           SchedKind sched = SCHED_JK_OUTER) {
    cur_V=0; cur_B=B;
    layout_2d_blocked(h_vn_ie,src_vn_ie,N,nlev,B); layout_2d_blocked(h_w,src_w,N,nlev,B);
    layout_2d_blocked(h_z_vt_ie,src_z_vt_ie,N,nlev,B); layout_2d_blocked(h_z_w_v,src_z_w_v,N,nlev,B);
    layout_idx_blocked(h_cidx,cell_logical,N,B); layout_idx_blocked(h_vidx,vert_logical,N,B);
#ifndef __CUDACC__
...
private:
  void _free_numa() {
    ...
    numa_dealloc(h_vn_ie,sz2d); numa_dealloc(h_w,sz2d);
    numa_dealloc(h_z_vt_ie,sz2d); numa_dealloc(h_z_w_v,sz2d);
    numa_dealloc(h_out,sz2d);
    numa_dealloc(h_cidx,(size_t)N*2); numa_dealloc(h_vidx,(size_t)N*2);
    numa_dealloc(inv_dual,(size_t)N); numa_dealloc(inv_primal,(size_t)N);
  ...
  void _numa_redistribute(int V, SchedKind sched) {
    ...
    switch(V){
    case 1:
      nv=redistribute_2d<1>(h_vn_ie,N,nlev,sched); nw=redistribute_2d<1>(h_w,N,nlev,sched);
      nvt=redistribute_2d<1>(h_z_vt_ie,N,nlev,sched); nzw=redistribute_2d<1>(h_z_w_v,N,nlev,sched);
      nc=redistribute_idx<1>(h_cidx,N,sched); nvi=redistribute_idx<1>(h_vidx,N,sched); break;
    case 2:
      nv=redistribute_2d<2>(h_vn_ie,N,nlev,sched); nw=redistribute_2d<2>(h_w,N,nlev,sched);
      nvt=redistribute_2d<2>(h_z_vt_ie,N,nlev,sched); nzw=redistribute_2d<2>(h_z_w_v,N,nlev,sched);
      nc=redistribute_idx<2>(h_cidx,N,sched); nvi=redistribute_idx<2>(h_vidx,N,sched); break;
    case 3:
      nv=redistribute_2d<3>(h_vn_ie,N,nlev,sched); nw=redistribute_2d<3>(h_w,N,nlev,sched);
      nvt=redistribute_2d<3>(h_z_vt_ie,N,nlev,sched); nzw=redistribute_2d<3>(h_z_w_v,N,nlev,sched);
      nc=redistribute_idx<3>(h_cidx,N,sched); nvi=redistribute_idx<3>(h_vidx,N,sched); break;
    default:
      nv=redistribute_2d<4>(h_vn_ie,N,nlev,sched); nw=redistribute_2d<4>(h_w,N,nlev,sched);
      nvt=redistribute_2d<4>(h_z_vt_ie,N,nlev,sched); nzw=redistribute_2d<4>(h_z_w_v,N,nlev,sched);
      nc=redistribute_idx<4>(h_cidx,N,sched); nvi=redistribute_idx<4>(h_vidx,N,sched); break;
    }
    double *nout = numa_alloc_unfaulted<double>(sz2d);
#pragma omp parallel for schedule(static)
    for (size_t i=0;i<sz2d;i++) nout[i]=0.0;
...
struct VertData {
  ...
  void init(int N, std::mt19937 &rng) {
    logical=new int[N*2]; int *tmp=new int[N];
    gen_permutation(tmp,N,rng); for(int i=0;i<N;i++) logical[i*2]=tmp[i];
    gen_permutation(tmp,N,rng); for(int i=0;i<N;i++) logical[i*2+1]=tmp[i];
    delete[]tmp;
  }
