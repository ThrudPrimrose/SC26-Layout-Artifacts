...
int main(int argc, char* argv[]) {
    ...
    for (int ni = 0; ni < N_NLEVS; ni++) {
        ...
        if (have_exact) {
            ...
            int *d_ci,*d_vi;
            CUDA_CHECK(hipMalloc(&d_vn, bd.sz_e*8));
            CUDA_CHECK(hipMalloc(&d_w,  bd.sz_c*8));
            CUDA_CHECK(hipMalloc(&d_vt, bd.sz_e*8));
            CUDA_CHECK(hipMalloc(&d_zw, bd.sz_v*8));
            CUDA_CHECK(hipMalloc(&d_out,bd.sz_e*8));
            CUDA_CHECK(hipMalloc(&d_id, N_e*8));
            CUDA_CHECK(hipMalloc(&d_ip, N_e*8));
            CUDA_CHECK(hipMalloc(&d_tg, N_e*8));
            CUDA_CHECK(hipMalloc(&d_ci, N_e*2*4));
            CUDA_CHECK(hipMalloc(&d_vi, N_e*2*4));
            hipEvent_t ev0, ev1;
