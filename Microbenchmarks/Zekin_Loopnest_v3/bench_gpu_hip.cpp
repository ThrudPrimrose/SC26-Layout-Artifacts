...
static void run_variant_configs(
    ...
    hipEvent_t ev0, hipEvent_t ev1, double* h_gpu_out)
{
    ...
    CUDA_CHECK(hipMemcpy(d_vn_ie,   bd.h_vn_ie,   bd.sz_e*8, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_w,       bd.h_w,       bd.sz_c*8, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_z_vt_ie, bd.h_z_vt_ie, bd.sz_e*8, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_z_w_v,   bd.h_z_w_v,   bd.sz_v*8, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_cidx,    bd.h_cidx,    N_e*2*4,   hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_vidx,    bd.h_vidx,    N_e*2*4,   hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_inv_dual,   bd.inv_dual,   N_e*8, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_inv_primal, bd.inv_primal, N_e*8, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_tangent,    bd.tangent_o,  N_e*8, hipMemcpyHostToDevice));

    for (int ci=0; ci<N_GCFG; ci++) {
        CUDA_CHECK(hipMemset(d_out, 0, sz_e*8));
        bool launched=true;
        ...
        CUDA_CHECK(hipMemcpy(h_gpu_out,d_out,sz_e*8,hipMemcpyDeviceToHost));
    ...
    hipEvent_t ev0, hipEvent_t ev1, double* h_gpu_out)
{
    ...
    for (int ci=0;ci<N_GCFG_V6;ci++){
        ...
        for(int r=0;r<WARMUP;r++){g_flush.flush();
            ...
            if(!launched)break; CUDA_CHECK(hipDeviceSynchronize());}
        ...
        for(int r=0;r<NRUNS;r++){
            g_flush.flush(); CUDA_CHECK(hipEventRecord(ev0));
            launch_gpu_v6<4>(ci,d_out,d_vn_ie,d_inv_dual,d_w,d_cidx,
                d_z_vt_ie,d_inv_primal,d_tangent,d_z_w_v,d_vidx,N_e,N_c,N_v,nlev,nlev_end,nlev_padded);
            CUDA_CHECK(hipEventRecord(ev1)); CUDA_CHECK(hipEventSynchronize(ev1));
            float ms=0; CUDA_CHECK(hipEventElapsedTime(&ms,ev0,ev1));
    ...
    double* icon_inv_dual, double* icon_inv_primal, double* icon_tangent)
{
    ...
    CUDA_CHECK(hipMalloc(&d_vn,bd.sz_e*8)); CUDA_CHECK(hipMalloc(&d_w,bd.sz_c*8));
    CUDA_CHECK(hipMalloc(&d_vt,bd.sz_e*8)); CUDA_CHECK(hipMalloc(&d_zw,bd.sz_v*8));
    CUDA_CHECK(hipMalloc(&d_out,bd.sz_e*8));
    CUDA_CHECK(hipMalloc(&d_id,N_e*8)); CUDA_CHECK(hipMalloc(&d_ip,N_e*8)); CUDA_CHECK(hipMalloc(&d_tg,N_e*8));
    ...
    hipFree(d_vn);hipFree(d_w);hipFree(d_vt);hipFree(d_zw);hipFree(d_out);
    hipFree(d_id);hipFree(d_ip);hipFree(d_tg);hipFree(d_ci);hipFree(d_vi);
    CUDA_CHECK(hipEventDestroy(ev0)); CUDA_CHECK(hipEventDestroy(ev1));
