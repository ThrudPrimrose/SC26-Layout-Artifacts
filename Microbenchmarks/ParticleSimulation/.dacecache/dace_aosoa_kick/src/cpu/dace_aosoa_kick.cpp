...
struct dace_aosoa_kick_state_t {
...
#include <chrono>
#include <chrono>
void __program_dace_aosoa_kick_internal(dace_aosoa_kick_state_t*__state, double * __restrict__ a, double * __restrict__ v, double dt, int64_t nb, int64_t vl)
{
            ...
            for (auto bi = 0; bi < nb; bi += 1) {
                for (auto li = 0; li < vl; li += 1) {
                    ...
                    }
                    {
                        ...
                        v[((((3 * bi) * vl) + li) + vl)] = __out;
                    }
                    {
                        double __in2 = dt;
                        double __in1 = a[((((3 * bi) * vl) + li) + (2 * vl))];
                        double __out;
    ...
    __state->report.add_completion("SDFG dace_aosoa_kick", "Timer", __dace_ts_start_0, __dace_ts_end_0, 0, -1, -1);
    __state->report.save(".dacecache/dace_aosoa_kick/perf", __HASH_dace_aosoa_kick);
}

DACE_EXPORTED void __program_dace_aosoa_kick(dace_aosoa_kick_state_t *__state, double * __restrict__ a, double * __restrict__ v, double dt, int64_t nb, int64_t vl)
{
    __program_dace_aosoa_kick_internal(__state, a, v, dt, nb, vl);
}

DACE_EXPORTED dace_aosoa_kick_state_t *__dace_init_dace_aosoa_kick(int64_t nb, int64_t vl)
{

    int __result = 0;
    dace_aosoa_kick_state_t *__state = new dace_aosoa_kick_state_t;
...
DACE_EXPORTED int __dace_exit_dace_aosoa_kick(dace_aosoa_kick_state_t *__state)
{
