...
//        -DSZ_N=1024 -DB=32 -DT=32 -DNRUNS=50 -DNWARM=5 \
...
//      fused_tiled             : row-major A, T×T tiling, heap-private y2
...
#define SZ_N 1024
#endif
#ifndef B           // block size (square) for mode 3
#define B 32
#endif
#ifndef T           // tile size (square) for mode 4
#define T 32
#endif
#ifndef NRUNS
#define NRUNS 50
...
static constexpr int NB = (N + B - 1) / B;        // number of blocks per dim
static constexpr int BSIZ = NB * NB * B * B;       // padded allocation size

template<bool outer_col, bool inner_col>
inline int blkidx(int i, int j) {
    const int bi = i / B, bj = j / B;
    const int r  = i % B, c  = j % B;
    const int block_off = outer_col ? (bj * NB + bi) : (bi * NB + bj);
    const int inner_off = inner_col ? (c * B + r)    : (r * B + c);
    return block_off * B * B + inner_off;
}
...
// y1: parallel on block-row bi (each bi owns y1[bi*B .. bi*B+B-1])
// y2: parallel on block-col bj (each bj owns y2[bj*B .. bj*B+B-1])
                         ...
                         double *y1, double *y2)
{
    ...
    for (int bi = 0; bi < NB; bi++) {
        const int ie = std::min(bi * B + B, N);
        for (int bj = 0; bj < NB; bj++) {
            const int je = std::min(bj * B + B, N);
            for (int i = bi * B; i < ie; i++) {
                double acc = 0.0;
                for (int j = bj * B; j < je; j++)
                        ...
                        FILE *fp)
{
    ...
    double e1 = max_relerr(y1w, y1ref, N);
    double e2 = max_relerr(y2w, y2ref, N);
    fprintf(stderr, "%s %s B=%d: y1_err=%.2e y2_err=%.2e\n",
            (e1 > 1e-9 || e2 > 1e-9) ? "FAIL" : "OK", v.tag, B, e1, e2);
                       ...
                       double *y1, double *y2)
{
    ...
    for (int ii = 0; ii < N; ii += T) {
        const int ie = std::min(ii + T, N);
        for (int jj = 0; jj < N; jj += T) {
            const int je = std::min(jj + T, N);
            for (int i = ii; i < ie; i++) {
                double acc = 0.0;
                for (int j = jj; j < je; j++)
                      ...
                      FILE *fp)
{
    ...
    double e1 = max_relerr(y1w, y1ref, N);
    double e2 = max_relerr(y2w, y2ref, N);
    fprintf(stderr, "%s tiled T=%d: y1_err=%.2e y2_err=%.2e\n",
            (e1 > 1e-9 || e2 > 1e-9) ? "FAIL" : "OK", T, e1, e2);
    delete[] y1w; delete[] y2w;
}
...
// Two variants
// ─────────────
//  fused_tiled           : row-major A, T×T tiling
...
// collapse(2) distributes (N/T)² tile pairs across threads.
...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        fprintf(stderr, "  3 = blocked A, B=%d, variants RR/RC/CR/CC\n", B);
        fprintf(stderr, "  4 = tiled loops, T=%d\n", T);
        fprintf(stderr, "  5 = OpenBLAS 2x dgemv\n");
        fprintf(stderr, "  6 = fused single-pass: tiled (T=%d) + blk_{RR,RC,CR,CC} (B=%d)\n", T, B);
        return 1;
    }
    ...
    case 2:
        ...
        break;
    case 3:
        fprintf(stderr, "mode 3: blocked A B=%d\n", B);
        for (auto &v : BLK_VARIANTS)
            run_blocked(v, A, x1, x2, y10, y20, y1ref, y2ref, fp);
        break;
    case 4:
        fprintf(stderr, "mode 4: tiled loops T=%d\n", T);
