    ...
    detail::coarsen_for<COARSEN, UNROLL_C>([&](int c) {
        ...
        if (gi < ngms) {
            ...
            for (int na=0; na<nat; na++) {
                ...
                int ij=ofs[na]; double a2r=0,a2i=0;
                for (int ih=0; ih<nh_nt; ih++) {
                    double a1r=0,a1i=0;
                    for (int jh=0; jh<nh_nt; jh++) {
                        int col=nij+ijt[IDX3(ih,jh,nt1-1,IJ_N1,IJ_N2)]-1;
                        int ix=col*QGM_R+gi;
                ...
                yr[nli]+=SOA_MUL_RE(a2r,a2i,xr,xi_);
                yi[nli]+=SOA_MUL_IM(a2r,a2i,xr,xi_);
            }
        }
    });
}
    ...
    detail::coarsen_for<COARSEN, UNROLL_C>([&](int c) {
        ...
        if (gi < ngms) {
            ...
            for (int na=0; na<nat; na++) {
                ...
                tr=xr;ti=xi_; xr=SOA_MUL_RE(tr,ti,e2r[i2],e2i[i2]); xi_=SOA_MUL_IM(tr,ti,e2r[i2],e2i[i2]);
                tr=xr;ti=xi_; xr=SOA_MUL_RE(tr,ti,e3r[i3],e3i[i3]); xi_=SOA_MUL_IM(tr,ti,e3r[i3],e3i[i3]);
                acr+=SOA_MUL_RE(a2r,a2i,xr,xi_);
                aci+=SOA_MUL_IM(a2r,a2i,xr,xi_);
            }
            int nli = nl[gi]-1;
    ...
    const double* __restrict__ eqr, const double* __restrict__ eqi)
{
    ...
    detail::coarsen_for<COARSEN,UNROLL_C>([&](int c){ acr[c]=0; aci[c]=0; });
    ...
    for (int na=0; na<nat; na++) {
        ...
        detail::coarsen_for<COARSEN,UNROLL_C>([&](int c){
            ...
            if (gi<ngms) {
                ...
                tr=xr;ti=xi_; xr=SOA_MUL_RE(tr,ti,e2r[i2],e2i[i2]); xi_=SOA_MUL_IM(tr,ti,e2r[i2],e2i[i2]);
                tr=xr;ti=xi_; xr=SOA_MUL_RE(tr,ti,e3r[i3],e3i[i3]); xi_=SOA_MUL_IM(tr,ti,e3r[i3],e3i[i3]);
                acr[c]+=SOA_MUL_RE(a2r,a2i,xr,xi_);
                aci[c]+=SOA_MUL_IM(a2r,a2i,xr,xi_);
            }
        });
        __syncthreads();
    }
    detail::coarsen_for<COARSEN,UNROLL_C>([&](int c){
        int gi=gs+c; if(gi<ngms){ int i=nl[gi]-1; yr[i]+=acr[c]; yi[i]+=aci[c]; }
    });
}
